...
class KnowledgeBasesApi {
  ...
  Future<ListDocumentsResponse?> listDocuments(String knowledgeBaseId, Map<String, dynamic>? params) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}/documents'), params: params);
  ...
  Future<DocumentResponse?> addDocument(String knowledgeBaseId, DocumentAddRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}/documents'), body: body, contentType: 'application/json');
  ...
  Future<KnowledgeSearchResponse?> searchKnowledge(KnowledgeSearchRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/v1/knowledge-bases/search'), body: body, contentType: 'application/json');
  ...
  Future<KnowledgeBaseListResponse?> listKnowledgeBases(Map<String, dynamic>? params) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/knowledge-bases'), params: params);
  ...
  Future<KnowledgeBaseResponse?> createKnowledgeBase(KnowledgeBaseCreateRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/v1/knowledge-bases'), body: body, contentType: 'application/json');
  ...
  Future<DocumentResponse?> getDocument(String knowledgeBaseId, String documentId) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}/documents/${documentId}'));
    return response is DocumentResponse ? response : null;
  }

  /// Delete document
  Future<void> deleteDocument(String knowledgeBaseId, String documentId) async {
    await _client.delete(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}/documents/${documentId}'));
  }

  /// Get knowledge base
  Future<KnowledgeBaseResponse?> getKnowledgeBase(String knowledgeBaseId) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}'));
    return response is KnowledgeBaseResponse ? response : null;
  }

  /// Delete knowledge base
  Future<void> deleteKnowledgeBase(String knowledgeBaseId) async {
    await _client.delete(ApiPaths.aiPath('/v1/knowledge-bases/${knowledgeBaseId}'));
  }
}
