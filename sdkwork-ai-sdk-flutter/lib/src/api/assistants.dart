...
class AssistantsApi {
  ...
  AssistantsApi(this._client);

  /// Get assistant
  Future<AssistantResponse?> getAssistant(String assistant_id) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/assistants/${assistant_id}'));
  ...
  Future<AssistantResponse?> updateAssistant(String assistant_id, AssistantCreateRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/v1/assistants/${assistant_id}'), body: body, contentType: 'application/json');
    return response is AssistantResponse ? response : null;
  }

  /// Delete assistant
  Future<AssistantDeleteResponse?> deleteAssistant(String assistant_id) async {
    final response = await _client.delete(ApiPaths.aiPath('/v1/assistants/${assistant_id}'));
  ...
  Future<AssistantListResponse?> listAssistants(Map<String, dynamic>? params) async {
    final response = await _client.get(ApiPaths.aiPath('/v1/assistants'), params: params);
  ...
  Future<AssistantResponse?> createAssistant(AssistantCreateRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/v1/assistants'), body: body, contentType: 'application/json');
    return response is AssistantResponse ? response : null;
  }
}
