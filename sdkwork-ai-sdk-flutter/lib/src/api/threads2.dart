...
class Threads2Api {
  ...
  Threads2Api(this._client);

  /// Submit tool outputs
  Future<SubmitToolOutputsPostResponse?> submitToolOutputs(String thread_id, String run_id, SubmitToolOutputsPostRequest body) async {
    ...
    return response is SubmitToolOutputsPostResponse ? response : null;
  }
  ...
  /// Create run
  Future<CreateRunPostResponse?> createRun(String thread_id, RunCreateRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/threads/${thread_id}/runs'), body: body, contentType: 'application/json');
    return response is CreateRunPostResponse ? response : null;
  }
