...
class ImageApi {
  ...
  Future<ImageGenerationResponse?> createImage(ImageGenerationRequest body) async {
    final response = await _client.post(ApiPaths.aiPath('/images'), body: body, contentType: 'application/json');
    return response is ImageGenerationResponse ? response : null;
  }

  /// Create image
  Future<ImageGenerationResponse?> createImageGenerations(ImageGenerationRequest body) async {
