...
public class ImageApi {
    ...
    public func createImage(body: ImageGenerationRequest) async throws -> ImageGenerationResponse? {
        let response = try await client.post(ApiPaths.aiPath("/images"), body: body)
        return response as? ImageGenerationResponse
    }

    /// Create image
    public func createImageGenerations(body: ImageGenerationRequest) async throws -> ImageGenerationResponse? {
