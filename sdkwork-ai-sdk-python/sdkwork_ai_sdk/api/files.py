...
class FilesApi:
    ...
    def upload_file(self, body: Optional[UploadFileRequest] = None, params: Optional[Dict[str, Any]] = None) -> OpenAiFileObject:
        """Upload file"""
        return self._client.post(f"/ai/v3/files", data=body, params=params)

    def retrieve_file_content(self, fileId: str) -> str:
        """Retrieve file content"""
        return self._client.get(f"/ai/v3/files/{fileId}/content")

    def retrieve_file(self, fileId: str) -> OpenAiFileObject:
        """Retrieve file"""
        return self._client.get(f"/ai/v3/files/{fileId}")

    def delete_file(self, fileId: str) -> OpenAiFileDeleteResponse:
        """Delete file"""
        return self._client.delete(f"/ai/v3/files/{fileId}")
