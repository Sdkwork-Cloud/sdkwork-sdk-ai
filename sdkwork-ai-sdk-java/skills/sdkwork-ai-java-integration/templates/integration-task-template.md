# SDK Integration Task Template

## Context

- Domain: ai
- Language: java
- SDK package: `sdkwork-ai-sdk-java`
- Target app path: <fill>
- Integration mode: <registry | local-path>

## Stage Plan

1. Stage 0 Discover
2. Stage 1 Wire Dependency
3. Stage 2 Configure Client
4. Stage 3 Minimal Viable Call
5. Stage 4 Advanced Capability (optional)
6. Stage 5 Verify and Handoff

## Commands

- Registry integration: `mvn dependency:get -Dartifact=com.sdkwork:ai-sdk:1.0.0`
- Local integration: `mvn install:install-file -Dfile=<path-to-jar> -DgroupId=com.sdkwork -DartifactId=ai-sdk -Dversion=1.0.0 -Dpackaging=jar`
- Stage 1 verification: `mvn -DskipTests compile`
- Stage 5 verification: `mvn test -DskipTests=false`

## Deliverables

- [ ] Dependency integrated
- [ ] Client configured
- [ ] Minimal API call path verified
- [ ] Advanced capabilities verified (if enabled)
- [ ] Handoff + rollback documented

